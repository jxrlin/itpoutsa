<x-Drv-dsh>
            ...
            <div class="flex flex-wrap justify-between items-center">
                <div class="flex justify-start items-center">
                    ...
                    <a href="https://flowbite.com" class="flex mr-4">
                        ...
                        <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Pout Sa Wholesale</span>
                    </a>
                    <form style="margin-left: 20px" action="#" method="GET" class="hidden lg:block lg:pl-2">
                        <label for="topbar-search" class="sr-only">Search</label>
                        <div class="relative mt-1 lg:w-96">
                            <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                                {{--                                <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20"> <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"/> </svg>--}}
                            </div>
                            <input type="text" style="padding-left: 12px" name="email" id="topbar-search" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full pl-9 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Search">
                        </div>
                    </form>
                </div>
    ...
    <div class="flex flex-row justify-center items-center gap-6">
        <div class="bg-white mt-5 rounded-2xl shadow-lg p-6 flex flex-col items-center max-w-lg w-full text-center">
            <img
                src="/placeholder.svg?height=120&width=120"
                alt="John Doe's profile"
                class="w-28 h-28 rounded-full object-cover border-4 border-blue-500 mb-4"
            >
            <h2 class="text-2xl font-bold text-gray-800">John Doe</h2>
            <div class="space-y-2 mt-3">
                <p class="text-sm text-gray-600"><span class="font-semibold">Rating:</span> ⭐ 4.8 / 5.0</p>
                <p class="text-sm text-gray-600"><span class="font-semibold">Trips:</span> 🚗 1250</p>
                <p class="text-sm text-gray-600"><span class="font-semibold">Experience:</span> 🎖️ 5 years</p>
            </div>
        </div>
        ...
        <div class="bg-white mt-5 rounded-2xl shadow-lg p-6 flex flex-col items-center max-w-lg w-full text-center">
            ...
            >
            <h2 class="text-2xl font-bold text-gray-800">Yangon, Myanmar</h2>
            <div class="space-y-2 mt-3">
                <p class="text-sm text-gray-600"><span class="font-semibold">Temperature:</span> 32°C</p>
                <p class="text-sm text-gray-600"><span class="font-semibold">Condition:</span> Partly Cloudy</p>
                <p class="text-sm text-gray-600"><span class="font-semibold">Humidity:</span> 75%</p>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-full p-6">
        ...
        <div class="border rounded-2xl shadow-lg bg-white p-6">
            <!-- Header Box -->
            <div class="flex items-center justify-center mt-10 bg-gradient-to-r from-blue-500 to-teal-500 text-white text-center py-6 mb-8 rounded-xl shadow-md">
                <h2 class="text-4xl font-extrabold tracking-tight">📦 Daily Delivery List</h2>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 table-auto">
                    ...
                    @foreach($deliveryLists as $index => $delivery)
                        <tr>
                            {{--                            <td class="px-6 py-4 whitespace-nowrap">--}}
                            {{--                                <div class="text-sm text-gray-900">{{ $delivery->salesInvoice->sales_invoice_id ?? 'No Shop Name' }}</div>--}}
                            {{--                            </td>--}}
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $delivery->salesInvoice->partnerShop->partner_shops_name ?? 'No Shop Name' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">{{ $delivery->salesInvoice->partnerShop->partner_shops_address ?? 'No Shop Address' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            <span id="status-{{ $index }}" class="px-3 py-1 text-sm font-semibold text-white rounded-full {{ $delivery->delivery_status == 'Sent' ? 'bg-green-500' : 'bg-red-500' }}">
                                {{ $delivery->delivery_status }}
                            </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <button id="btn-{{ $index }}" onclick="openFirstModal({{ $index }}, '{{ $delivery->salesInvoice->partnerShop->partner_shops_name ?? 'No Shop' }}', '{{ $delivery->salesInvoice->partnerShop->partner_shops_address ?? 'No Address' }}', '{{ $delivery->driver->phone_number ?? 'No Phone Number' }}', '{{ $delivery->id }}')"
                                        class="px-5 py-2 text-sm font-medium text-white bg-blue-500 rounded-lg shadow-md hover:bg-blue-600 transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                                    {{ $delivery->delivery_status == 'Sent' ? 'disabled' : '' }}>
                                    Send
                                </button>
                            </td>
                        </tr>
    ...
    <!-- Modern Modal -->
    <div id="firstModal" class="modal-overlay fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4 z-50">
        <div class="modal-content bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-md shadow-xl">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Confirm Delivery Details</h3>
            ...
            <p id="invoiceDetails" class="text-gray-600 dark:text-gray-300 mb-4"></p>
            <textarea id="messageBox" class="w-full p-3 border rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white resize-none focus:ring-2 focus:ring-blue-500 transition-all" rows="4" readonly></textarea>
            <div class="flex gap-3 mt-6 justify-end">
                <button onclick="closeModal()" class="px-4 py-2 bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-white rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors">Cancel</button>
                <button id="confirmPhone" onclick="confirmPhone()" class="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors">Confirm Phone</button>
                <button id="sendMessage" onclick="sendMessage()" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">Send Message</button>
            </div>
        </div>
    </div>

    <!-- Toast Notification -->
    <div id="toastMessage" class="fixed bottom-5 right-5 bg-green-600 text-white px-4 py-2 rounded-lg shadow-lg hidden">Message sent successfully!</div>
</x-Drv-dsh>
